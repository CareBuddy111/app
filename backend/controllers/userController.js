...
const bookAppointment = async (req,res)=>{
    try {
        ...
        if (!docData.available) {
            return res.json({success:false,message:"Doctor not availabile"})
        }
        ...
        if (slots_booked[slotDate]) {
            if (slots_booked[slotDate].includes(slotTime)) {
                ...
                return res.json({success:false,message:"Slot not availabile"})
            } else{
        ...
        const newAppointmrnt = new appointmentModel(appointmentData)
        await newAppointmrnt.save()

            // save new slots data in doctodata
...
// API TO GET USER APPOINTFOR FRONEND MY APPOINTMENT PAGE
